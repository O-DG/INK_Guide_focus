...
function i_guide_focus() {
  ...
  // 가이드 활성화
  if (guide_switch && !getCookie("i_guide_focus")) {
    // 임시적 쿠키로 적용함 > DB로 적용할 것을 권장
    // 클래스 추가(하이라이트 요소)
    ...
    if (guide_arr.length > 0) {
      ...
      // 하이라이팅 클론 요소
      let clone_el = clone_highlighting(current_info);

      // 하이라이팅 포커스
      focus_highlighting(current_info, clone_el);

      // 위치 동기화
      window.addEventListener("scroll", () => {
        guide_scroll_event(current_el, clone_el);
      });
      ...
      // 가이드 버튼 컨트롤러 이벤트
      guide_btn_close_keep.addEventListener("click", () => {
        // 더 이상 확인하지 않음
      ...
      //
      guide_btn_close.addEventListener("click", () => {
        // 닫기
        guide_close(guide_arr);
      });
      guide_btn_prev.addEventListener("click", () => {
        // 이전
      ...
      guide_btn_next.addEventListener("click", () => {
        // 다음
        // 비활성화 상태라면 이벤트 내용을 실행하지 않는다.
  ...
  function guide_highlighting(move = "next") {
    ...
    // 하이라이팅 클론 요소
    let clone_el = clone_highlighting(return_info);
    // 위치 동기화
    window.addEventListener("scroll", () => {
      guide_scroll_event(return_info.el, clone_el);
    });
    window.addEventListener("resize", () => {
      guide_scroll_event(return_info.el, clone_el);
    });
    ...
    // 하이라이팅 포커스
    focus_highlighting(return_info, clone_el);
  ...
  // 하이라이팅 포커스
  function focus_highlighting(f_info, clone_el) {
    // 초기화
    ...
    clone_el.appendChild(guide_focus_box);
    let el_rect = f_info.el ? f_info.el.getBoundingClientRect() : null;

    // guide_focus_box.classList.remove('active');
    guide_focus_box.classList.add("active");
    // guide_focus_box.style.cssText = `left:50%; top:50%`;
    guide_focus_box.style.cssText = `left:50%; top:0`;
  ...
  // 클론 위치 동기화
  function guide_scroll_event(el, clone_el) {
    let el_rect = el.getBoundingClientRect();
    // 위치 갱신
    clone_el.style.left = el_rect.left + "px";
    clone_el.style.top = el_rect.top + "px";
  }
}
